...
const byte CON_PASS[3] = {100, 80, 65};
...
byte diff[4] = {15, 20, 40, 40};

byte posSet[6] = {2, 3, 4, 5, 20, 100};

byte LOW_PASS[3] = {CON_PASS[0], CON_PASS[1], CON_PASS[2]};
...
void loop() { if (millis() - timer > 0){ 
  ...
  for (int i=0; i<4; i++){      //зменшую нижній поріг чувтсвітельності;
                                //вичісляю oldMax[i]; 
                                //очіщаю maxValue[i];
   oldmax[i] = (oldmax[i] + maxValue[i])/2;
   maxValue[i] = 0; 
   
  /*
   if (oldmax[i] < diff[i]){
    if (LOW_PASS[i] > CON_PASS[i]-10) LOW_PASS[i]--;
   } else if (LOW_PASS[i] < CON_PASS[i]) LOW_PASS[i]++; 
  }
  */
  
  for (int i = 2; i < 128; i++) {   //виключаю столбікі які нище чім Low_Pass;
                                    //усілюю, зменшую високі частоти;
                                    //вичісляю общій максімум;
...
//      fht_log_out[i] = (float)fht_log_out[i] / ((float)1 + (float)i / 128);
  ...
  if (value[0] > oldValue[0] && value[0] - oldValue[0] > 5) value[0] = oldValue[0] * Sbu + value[0] * (1 - Sbu); else if(oldValue[0] - value[0] > 5) value[0] = oldValue[0] * Sbd + value[0] * (1 - Sbd); 
  if (value[1] > oldValue[1] && value[1] - oldValue[1] > 5) value[1] = oldValue[1] * Smu + value[1] * (1 - Smu); else if(oldValue[1] - value[1] > 5) value[1] = oldValue[1] * Smd + value[1] * (1 - Smd);
  if (value[2] > oldValue[2] && value[2] - oldValue[2] > 5) value[2] = oldValue[2] * Shu + value[2] * (1 - Shu); else if(oldValue[2] - value[2] > 5) value[2] = oldValue[2] * Shd + value[2] * (1 - Shd);
...
//    /*
    for (int i=0; i<4; i++){
      Serial.print("oldMax[i]: ");
      Serial.println(oldmax[i]);
    }
      Serial.println(" "); 
//    */
...
void range(){             //рахую середнє арифметичне між стовпчікав >0
                          //вираховую maxValue  
